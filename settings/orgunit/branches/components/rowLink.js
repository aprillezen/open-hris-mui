...
import { loadDeleteDialog, cancelDelete, deleteBranch} from '../actions'

class RowLink extends Component{

	handleClick(e){		
		this.context.router.push('/settings/unit/branch/'+this.props.rowData.id)
	}

	handleDelete(e){
		this.props.dispatch(loadDeleteDialog(this.props.rowData.id, this.props.rowData.branchname))
	}
	...
	deleteNow(e){		
		this.props.dispatch(deleteBranch(this.props.deleteId))
	}
		...
		return(
			  <div className="rowLinkBorder">	
			 		 <table className="griddle-row-table">
						<tbody>
							<tr>
								<td className="griddle-row-table-td2"><h4>{this.props.rowData.branchname} </h4></td>								
...
const mapStateToProps =(state)=>{		
	return{		
		isDeleteDialogOpen: state.branch.isDeleteDialogOpen,
		isDeleting: state.branch.isDeleting,
		deleteErrorMsg: state.branch.deleteErrorMsg,
		deleteHasError: state.branch.hasError,
		deletemsg: state.branch.deletemsg,
		deleteId: state.branch.deleteId
	}
}
